...
func hasEverExisted(hash: String, pageID: String, context: ModelContext) throws -> Bool {
    let desc = FetchDescriptor<SyncUserContentPage>(
        predicate: #Predicate { $0.hashed == hash && $0.pageID == pageID }
    )
    return try context.fetch(desc).first != nil
}
